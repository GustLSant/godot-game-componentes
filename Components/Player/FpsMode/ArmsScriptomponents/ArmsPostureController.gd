...
func handleCrouchPosition() -> void:
	var targetPosY: float = int(playerState.isCrouched) * playerState.CROUCH_HEIGHT + int(playerState.isCrouched and not playerState.isAiming) * 0.3
	var targetPosZ: float = int(playerState.isCrouched) * 0.3
